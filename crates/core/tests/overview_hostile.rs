...
//!
//! Moved here from `crates/core/src/overview/hostile.rs` (#457): the suite
//! only ever drove the pipeline through its public API (`convert_to_overviews`,
//! `export_pmtiles`, `OverviewReader`, ...) with real parquet I/O, each case
//! run through both the in-memory and streaming engines -- exactly the kind
//! of full-pipeline test that used to make `cargo test --lib` a trap. As a
//! `tests/*.rs` integration binary it no longer runs under `--lib`. It is
//! fast (tiny synthetic fixtures), so it runs in nextest's `quick` profile
//! and therefore also `full` (see `.config/nextest.toml`), and under plain
//! `cargo test`.
//!
//! A handful of white-box tests that exercise `pub(super)` helpers in
//! `overview::convert` (bbox/CRS classification, the all-lost gate) could not
//! move -- those items are not part of the crate's public API -- and remain
//! in `crates/core/src/overview/hostile.rs` as a small, fast unit-test module.
//!
//! `write_input`/`write_input_with_f64` come from `overview::testutil`,
//! included by `#[path]` below (that module is `pub(crate)`, so it cannot be
//! imported through the crate's public API).
...
use tylertoo_core::overview::convert::{
    convert_to_overviews, ConvertError, ConvertOptions, LevelPlan,
};
use tylertoo_core::overview::export::{export_pmtiles, ExportError, ExportOptions};
use tylertoo_core::overview::level::Mode;
use tylertoo_core::overview::reader::{OverviewReader, ReaderError};
use tylertoo_core::overview::simplify::{CollapseMode, SimplifyOptions};

// Shared fixture writers, compiled straight from the crate's in-tree test
// module rather than copied: `overview::testutil` is `pub(crate)` (invisible
// to this integration-test crate), but it depends only on std and external
// crates, so `#[path]` can include the same source here and the two can't
// drift. Not every helper in it is used by this binary.
#[allow(dead_code)]
#[path = "../src/overview/testutil.rs"]
mod testutil;

use testutil::{write_input, write_input_with_f64};
...
fn points_at_the_max_zoom_ceiling_convert() {
    ...
    let o = ConvertOptions {
        levels: LevelPlan::ZoomRange {
            min_zoom: 28,
            max_zoom: tylertoo_core::tile::MAX_ZOOM,
        },
        ..Default::default()
    };
    let report = convert_to_overviews(tin.path(), tout.path(), &o).unwrap();
...
fn archive_layer_fields(path: &Path) -> Vec<String> {
    ...
    let json = tylertoo_core::compression::decompress_capped(
        &buf[off..off + len],
        tylertoo_core::compression::Compression::Gzip,
        tylertoo_core::compression::MAX_INTERNAL_BYTES,
    )
...
fn write_banded_input(path: &Path, sites: usize) -> Vec<f64> {
    ...
    write_input_with_f64(path, &geoms, "magnitude", &values);
    values
}
...
fn property_selection_narrows_the_overview_columns() {
    use tylertoo_core::overview::properties::PropertySelection;
...
fn property_selection_makes_the_source_single_use() {
    ...
    use tylertoo_core::overview::convert::convert_to_overviews_sources;
    use tylertoo_core::overview::properties::PropertySelection;
...
fn tiny_polygon_accumulator_stays_out_of_point_bands() {
    use tylertoo_core::overview::convert::RepresentationBand;
    use tylertoo_core::overview::simplify::Representation;
