...
//! part of nextest's `full` profile (see `.config/nextest.toml`) and plain
//! `cargo test`.
//!
...
//!
//! `write_input`/`write_input_with_f64` below are a deliberate duplicate of
//! `overview::testutil`'s fixture writers: that module is `pub(crate)` and
//! invisible from this integration-test crate, which links only against
//! tylertoo-core's public API.
