...
///
/// Generate tiles from multiple GeoParquet files (partitioned data) with progress reporting.
///
/// Processes each file sequentially using parallel row group reading internally.
/// All tiles are written to a single PMTiles output.
#[allow(clippy::type_complexity)]
#[instrument(name = "pipeline_multi", skip(writer, progress), fields(num_files = input_paths.len(), min_zoom = config.min_zoom, max_zoom = config.max_zoom))]
pub fn generate_tiles_from_partitions_with_progress(
    ...
    for (file_idx, input_path) in input_paths.iter().enumerate() {
        ...
        // Clone the Arc for this file
        let progress_clone = Arc::clone(&progress_arc);
        let file_progress: ProgressCallback = Box::new(move |event| progress_clone(event));

        let file_stats =
            generate_tiles_to_writer_internal(input_path, config, writer, Some(file_progress))?;

        // Accumulate peak memory (take max across files)
        peak_bytes = peak_bytes.max(file_stats.peak_bytes);
    }

    // Return stats with peak memory from all files
