...
| `.cz.toml` | `version` |
| `crates/python/uv.lock` | `version` under `name = "tylertoo"` (regenerated by `uv lock`, not by `cz bump`) |
...
# It does NOT update crates/python/uv.lock; the pre-commit
# hook runs `uv lock` and stages it. With hooks disabled, run
# it yourself or CI's `uv sync --locked` fails:
cd crates/python && uv lock
