...
tylertoo vs the geoparquet-io recommended pipeline (GeoJSON → tippecanoe).
...
- **Machine:** 16 cores.
- **Versions:** tippecanoe v2.49.0, geoparquet-io 1.1.0b1, tylertoo built from
  the #256 branch (simple-clip fast path default-on).
- **Fairness:** matched across pipelines — zoom `z0–14`, layer `buildings`, and
  a 500K per-tile cap (tippecanoe's default; tylertoo pinned via
  `--max-tile-size 500K`).
...
```bash
# tylertoo — native, no intermediate
...
**1. Native beats the round-trip by an order of magnitude.** tylertoo turns the
...
**2.5× tylertoo' entire end-to-end time** — before tippecanoe writes a single
tile.
...
(tippecanoe's drop-to-fit flag). tylertoo applies a drop-to-fit budget **by
...
**4. tylertoo default vs tuned is a *quality* knob, not a speed one.** The tuned
...
- Wall time: `/usr/bin/time -v` for the single-process tylertoo runs; pipeline
  wall clock for the streamed gpio → tippecanoe run.
- Peak RSS: `time -v` maximum-RSS for tylertoo; a 0.5 s process-group sampler
