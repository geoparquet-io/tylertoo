...
* `in_flight_batches` (`int`) — Read batches allowed in flight through the pass-2 pipeline at once (read/compute-overlap knob). Higher improves core utilization at proportionally more peak memory. Defaults to 0, which auto-sizes to the machine's available cores (clamped to 4..=16); pass an explicit positive integer to override.
