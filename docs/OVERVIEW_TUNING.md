...
⚠️ **musl binaries before 0.8 — prefer `--profile bounded` on large
...
0.8 the musl binary ships with mimalloc as its global allocator and no longer
needs the workaround; other platforms were never affected.
...
| `memory allocation of N bytes failed` on a pre-0.8 musl binary, with RAM to spare | `--profile bounded` — musl `mallocng` fragmentation, fixed in 0.8 by shipping mimalloc ([#480](https://github.com/geoparquet-io/tylertoo/issues/480)) |
