...
| Sub-pixel polygons at coarse zooms | Hard-dropped by default (visibility gate + write-time RDP collapse); opt-in `--collapse` → representative Point (spec Q4) | Tiny-polygon reduction ON by default: accumulates dropped area, emits placeholder squares | Type-preserving default keeps renderers unsurprised, but a dense small-polygon layer is EMPTY at country zooms out of the box (#259); the tippecanoe-parity placeholder-square mechanism is future work (old per-tile-pipeline design: #85, removed with #177; structural fix: #246) |
