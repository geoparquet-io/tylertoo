...
| Sub-pixel polygons at coarse zooms | Hard-dropped by default; opt-in dispositions: `--collapse` → representative Point (spec Q4), `--collapse-square` → area-dithered ~1×GSD placeholder square (#279) | Tiny-polygon reduction ON by default: accumulates dropped area serially per tile, emits placeholder squares | Type-preserving drop default keeps renderers unsurprised and is unchanged pending the #259-fixture sweep (#279 tracks the default decision). Our squares dither **per feature** (deterministic hash of the anchor coordinates, keep probability `area/tol²`) instead of tippecanoe's serial per-tile accumulator: levels have no tile scope, and three engines with different batch/thread schedules must stay byte-identical — same aggregate area in expectation, order-independent. Area removed by cell-winner thinning / the density budget is NOT accumulated (a thinned feature emits no square). Old per-tile-pipeline accumulator design: #85, removed with #177; structural fix: #246 |
