...
| Stable MVT feature ids (export, `--feature-id`, #443) | Null/negative/non-integer values in the named column are hard export errors naming the column and the row's file scan order | `--use-attribute-for-id` only *warns* on a non-numeric or fractional id and, for a numeric string, feeds it through `strtoull`: a leading `-` computes the value's two's-complement unsigned reinterpretation and is accepted whenever printing it back out reproduces the input string (`serial.cpp`), so `"-5"` silently becomes id `18446744073709551611` with no error | Issue #443 asks for hard errors instead of that silent reinterpretation. Matched: the named column is moved to the feature id and never *also* published as a regular property (tippecanoe's fixed `--use-attribute-for-id` behaviour, mapbox/tippecanoe#738), independent of `--include-property`/`--exclude-property` naming the same column. The no-flag default (an MVT id unique only within one tile/zoom, `i as u64` in `build_mvt`) is unchanged pending #443's own open question: whether it should instead omit the id entirely |
